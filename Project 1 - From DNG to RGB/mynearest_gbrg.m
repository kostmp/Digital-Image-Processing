%similar to mynearest.m but for gbrg bayer type
function rgbim = mynearest_gbrg(red_image,green_image,blue_image,M,N)
for i=1:M
    if mod(i,2) == 1
        for j=2:2:N
            green_image(i,j) = green_image(i,j-1);
            blue_image(i,j-1) = blue_image(i,j);
            red_image(i,j) = red_image(i+1,j-1);
            red_image(i,j-1) = red_image(i+1,j-1);
        end
    elseif mod(i,2) == 0
        %for j=1:imageHeight
